...
const getPost = (id) => new Task((rej, res) => setTimeout(() => (posts[id] ? res(posts[id]) : rej("not found")), 200));

const getComments = (post) => new Task((rej, res) => setTimeout(() => res(comments[post.title]), 200));
...
// =========================
const getHref = 
new Task((rej,res) =>location.href ? res(location.href) : rej(console.error))
